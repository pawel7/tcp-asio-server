...
public:
    client(std::string server, short port) : server_(server),
                                             port_(port)
    {
        ...
        while (request_length_ > 0) // wybrano z menu coś poza "Koniec"
        {
            ...
            asio::write(socket, asio::buffer(request_, (size_t)request_length_), error);
            if (!error)
            {
            ...
            {
                const char *data = asio::buffer_cast<const char *>(response_buffer.data());
        ...
        do
        {
            ...
            case 4:
                ...
                len = sprintf(request_,
                              ...
                              "Connection: close\r\n\r\n"
                              "%s",
                              nr_temperatury, post_html_len, post_html_buf);
                break;
